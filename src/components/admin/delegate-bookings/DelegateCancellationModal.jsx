...
export default function DelegateCancellationModal({
  show,
  booking,
  onClose,
  onConfirm,
        ...
        <div className={`relative max-w-3xl w-full mx-auto rounded-xl shadow-2xl overflow-hidden transform transition-all z-[70] ${
          ...
          <div className="bg-gradient-to-r from-red-600 to-pink-600 px-6 py-4 flex items-center justify-between">
            <div className="flex items-center">
              ...
              <h3 className="text-xl font-bold text-white">❌ Cancel Delegate Registration</h3>
            </div>
            <button
              onClick={onClose}
              ...
              <div className={`p-4 rounded-lg ${
                ...
                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Registration ID:</span>
                    <span className={`text-xs font-mono ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} max-w-[180px] truncate`}>
                      {booking.id}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Delegate:</span>
                    <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} max-w-[180px] truncate`}>
                      {booking.delegateDetails?.name || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Organization:</span>
                    <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} max-w-[180px] truncate`}>
                      {booking.delegateDetails?.organization || 'N/A'}
                    </span>
                  </div>
                  <div className="flex justify-between items-center">
          ...
          <div className={`px-6 py-4 border-t ${isDarkMode ? 'border-gray-700 bg-gray-750' : 'border-gray-200 bg-gray-50'}`}>
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center justify-center min-w-[140px]"
              >
                {isUpdating ? (
                  <>
